...
from amuse.plot import convert_particles_to_pynbody_data, HAS_PYNBODY, scatter
...
FRAME_PRINT = 10#0 # Print animation update every x frames

############################################################################
### A: Evolution
############################################################################

def animate_HR(HR_data, parameters, filepath="test_HR_animation.gif"):
    """
    ...
    temp_lim = [0.75*np.min(temps), 1.25*np.max(temps)]
    lum_lim = [0.75*np.min(lums), 1.25*np.max(lums)]
    ...
    gif.save(filepath, writer = animation.PillowWriter(fps=30))
    print("Done.")
...
# old with recent updates
def plot_particles_2D(core, gas, i, parameters, filepath="test_relax_plot2D.png"):
    ...
    ax.add_patch(plt.Circle(core_pos, core_radius, color="red", fill=False, zorder=10))
    ax.add_patch(plt.Circle(core_pos, roche_radius, color="black", fill=False, zorder=10))
    ax.add_patch(plt.Circle(core_pos, stop_radius, color="black", linestyle = "dashed", fill=False, zorder=10))

    ax.scatter(gas.x.value_in(units.RSun), gas.y.value_in(units.RSun), alpha=0.5, s=1.5, linewidth=0)

    ax.set_xlabel(r"X [R$_\odot$]")
    ax.set_ylabel(r"Y [R$_\odot$]")
    ax.set_title(f"Step {i:05}; Time {parameters['relax_timestep_day'] * i:.2f}")
...
#     if "periapsis_RSun" in parameters:
#         roche_radius = estimate_roche_radius_periapsis_parameters(parameters).value_in(units.RSun)
#     else:
#         roche_radius = estimate_roche_radius_parameters(parameters).value_in(units.RSun)
#     stop_radius = roche_radius * parameters["roche_multiplier"]
...
def energy_evolution_plot(times, potential, kinetic, thermal, filepath="energy_evolution.png"):
    ...
    ax1.plot(times, kinetic, color="blue")
    ax1.set_ylabel("Kinetic [J]")

    ax2.plot(times, potential, color="orange")
    ax2.set_ylabel("Potential [J]")

    ax3.plot(times, thermal, color="green")
    ax3.set_ylabel("Thermal [J]")

    ax4.plot(times, (kinetic+potential+thermal), color="red")
...
def potential_energy_plot(time, potential, filepath="potential_energy_evolution.png"):
    ...
    plt.plot(time, potential, color = "orange")
...
def plot_relaxation_energies(dirs, parameters):
    ...
    energy_files = [f for f in os.listdir(dirs["data"]) if f.startswith("energies")]
    if len(energy_files) > 0:
        print(f"\nLoading previous energy values...")
        energy_path = os.path.join(dirs["data"], energy_files[0])
        #
        contents = np.loadtxt(energy_path, unpack=True)
        if len(contents) == 5:
            times, kinetic, potential, thermal, total = contents
        else:
            times, kinetic, potential, thermal = contents

    else:
        print(f"\nObtaining previous energy values from snapshots...")
    ...
    filepath = os.path.join(dirs["plots"], f"energy_evolution.png")
    ...
    filepath = os.path.join(dirs["plots"], f"potential_energy.png")
    potential_energy_plot(times, potential, filepath=filepath)
...
def animate_simulation(gas_snapshots, dm_snapshots, dirs, parameters, fps=15, vmin=1e-3, vmax=1e2, resolution=500, filepath="test_simulation_animation.gif"):
    """
    Creates an animation of simulation in both particles and density figures.
    :gas_snapshots:  Snapshots of the SPH particles
    ...
    # Size of the image
    amuse_unit = units.RSun
    pynbody_unit = f"{amuse_unit.value_in(units.m)} m"
    width = 2.5 * apoapsis_from_periapsis(parameters["eccentricity"], parameters["periapsis_RSun"] | units.RSun).value_in(amuse_unit)
    extent = (-width/2, width/2, -width/2, width/2)

    fig, (ax_sph, ax_rho) = plt.subplots(1, 2, sharex=True, sharey=True, figsize = (15,7), constrained_layout=True)
    ...
    gas_x = sph_particles.x.value_in(amuse_unit)
    gas_y = sph_particles.y.value_in(amuse_unit)
    core_pos = core.position.value_in(amuse_unit)
    sec_pos = sec.position.value_in(amuse_unit)

    # Radius of the various circles
    core_radius = core.radius.value_in(amuse_unit)
    roche_radius = estimate_roche_radius_periapsis_parameters(parameters).value_in(amuse_unit)
    sec_radius = (sec.mass.value_in(units.MSun)**0.8 | units.RSun).value_in(amuse_unit)  # Main-sequence mass-radius relationship
    sink_radius = parameters["sink_multiplier"] * sec_radius

    particles = ax_sph.scatter(gas_x, gas_y, alpha=1, s=2, linewidth=0, zorder = 2)

    # Various circles representing radii
    core_circ = ax_sph.add_patch(plt.Circle(core_pos[:2], core_radius, color="red", fill=True, zorder=9))
    roche_circ = ax_sph.add_patch(plt.Circle(core_pos[:2], roche_radius, color="black", linestyle="dashed", fill=False, zorder=9))
    sec_circ = ax_sph.add_patch(plt.Circle(sec_pos[:2], sec_radius, color="black", fill=True, zorder=9))
    sink_circ = ax_sph.add_patch(plt.Circle(sec_pos[:2], sink_radius, color="black", linestyle="dashed", fill=False, zorder=9))

    ax_sph.set_xlabel(r"X [$R_\odot$]")
    ax_sph.set_ylabel(r"Y [$R_\odot$]")

    ax_sph.set_xlim(extent[:2])
    ax_sph.set_ylim(extent[-2:])

    # ax_sph.set_aspect("equal")

    ###########
    # Density #
    ###########

    # Options for pynbody image
    qty = "rho"
    qty_unit = "kg m^-3"
    cmap = "hot"

    pyndata = convert_particles_to_pynbody_data(sph_particles, amuse_unit, pynbody_unit)
    image_data = pynbody_sph.image(pyndata, qty=qty, units=qty_unit, width=width, resolution=resolution, noplot=True, ret_im=False)
    image_data[np.where(image_data < vmin)] = 0.1 * vmin # Removes artifacts from numbers too close to 0

    image = ax_rho.imshow(image_data, norm=LogNorm(vmin=vmin, vmax=vmax), cmap=cmap, origin="lower", extent=extent)
    plt.colorbar(image, ax=ax_rho, label = r"Density [kg m$^{-3}$]", pad=0.015)

    dm_circs = ax_rho.scatter(dm_particles.x.value_in(amuse_unit), dm_particles.y.value_in(amuse_unit), color = "white", edgecolor="black")

    ax_rho.set_xlabel(r"X [$R_\odot$]")
    ...
    def update(frame):
        ...
        gas_pos = np.dstack((sph_particles.x.value_in(units.RSun), sph_particles.y.value_in(units.RSun)))[0]
        core_pos = np.array((core.x.value_in(units.RSun), core.y.value_in(units.RSun)))
        sec_pos = np.dstack((sec.x.value_in(units.RSun), sec.y.value_in(units.RSun)))[0][0]

        particles.set_offsets(gas_pos)
        core_circ.set(center=core_pos)
        roche_circ.set(center=core_pos)
        sec_circ.set(center=sec_pos)
        sink_circ.set(center=sec_pos)

        # Update density plot
        dm_circs.set_offsets(np.dstack((dm_particles.x.value_in(amuse_unit), dm_particles.y.value_in(amuse_unit)))[0])
    ...
    # Save the animation
    print(f"Animating simulation ({N_frames} frames)")
    gif.save(filepath, writer = animation.PillowWriter(fps=fps))

    plt.close()

def animate_simulation_density(gas_snapshots, dm_snapshots, dirs, parameters, fps=15, filepath="test_density_animation.gif"):
    """
    ...
    vmin = 1e-4
    vmax = 1e2
    resolution = 500 # Dimensions of the image in pixels
    cmap = "hot"
    ...
    image = ax.imshow(image_data, norm=LogNorm(vmin=vmin, vmax=vmax), cmap=cmap, origin="lower", extent=extent)
    plt.colorbar(image, ax=ax)

    dm_circs = ax.scatter(dm_particles.x.value_in(amuse_unit), dm_particles.y.value_in(amuse_unit), color = "white", edgecolor="black")
    ...
    def update(frame):
        if frame > 0 and frame % FRAME_PRINT == 0:
            ...
            print(f"{frame:5}: {frame * parameters['simulation_timestep_day']:.2f} days | time elapsed = {new_time - old_time:.3f} s")
...
def animate_simulation_particles(gas_snapshots, dm_snapshots, dirs, parameters, fps=15, filepath="test_sim_animation.gif", do_trail=False):
    ...
    # Various circles representing radii
    core_circ_xy = ax_xy.add_patch(plt.Circle(core_pos[:2], core_radius, color="red", fill=True, zorder=9))
    roche_circ_xy = ax_xy.add_patch(plt.Circle(core_pos[:2], roche_radius, color="black", linestyle="dashed", fill=False, zorder=9))
    sec_circ_xy = ax_xy.add_patch(plt.Circle(sec_pos[:2], sec_radius, color="black", fill=True, zorder=9))
    sink_circ_xy = ax_xy.add_patch(plt.Circle(sec_pos[:2], sink_radius, color="black", linestyle="dashed", fill=False, zorder=9))
    ...
    core_circ_xz = ax_xz.add_patch(plt.Circle(core_pos[::2], core_radius, color="red", fill=True, zorder=9))
    ...
    sink_circ_xz = ax_xz.add_patch(plt.Circle(sec_pos[:2], sink_radius, color="black", linestyle="dashed", fill=False, zorder=sec_zorder))
    ...
    def update(frame):
        ...
        sink_circ_xy.set(center=sec_xy)
        ...
        sink_circ_xz.set(center=sec_xz, zorder=sec_zorder)
...
def plot_multi_sim_density(gas_snapshots, dm_snapshots, rows, columns, indeces, dirs, parameters, filepath="test_multi_density.png"):
...
def post_relaxation_visualisations(dirs, parameters):
    ...
    # Plots relaxation
    animate_relaxation_2D(snapshots_core, snapshots_gas, dirs, parameters, fps=10, filepath=os.path.join(dirs["plots"], "animation_relaxation_2D.gif"))
...
def animate_relaxation_2D(core_snapshots, gas_snapshots, dirs, parameters, fps=15, filepath="test_relax_animation2D.gif"):
    ...
    gas_file = os.path.join(dirs["snapshots"], gas_snapshots[0])
    gas = read_set_from_file(gas_file, format="amuse")
    particles = ax.scatter(gas.x.value_in(units.RSun), gas.y.value_in(units.RSun), alpha=0.5, s=2, linewidth=0)
    ...
    core_radius = core.radius.value_in(units.RSun)
    core_pos = (core.x.value_in(units.RSun), core.y.value_in(units.RSun))
    core_circ = ax.add_patch(plt.Circle(core_pos[:2], core_radius, color="red", fill=True, alpha=0.7, zorder=10))

    # Draw a circle representing the Roche radius and the stop-evolve radius
    roche_circ = ax.add_patch(plt.Circle(core_pos[:2], roche_radius, color="black", linestyle = "dashed", linewidth=2, fill=False, zorder=10))
    # stop_circ = ax.add_patch(plt.Circle(core_pos[:2], stop_radius, color="black", linestyle = "dashed", fill=False, zorder=10))
    ...
    limit = 1.1 * stop_radius
    plt.xlim([-limit, limit])
    plt.ylim([-limit, limit])

    plt.xticks([-8,-4,0,4,8], [-8,-4,0,4,8])
    ...
    # Create the animation
    N_frames = len(core_snapshots[:30])
    gif = animation.FuncAnimation(fig=fig, func=update, frames=N_frames)
...
if __name__ == "__main__":
    ...
    working_directory = "RL0.8_Mp1.5_Ms1.4_e0.25_a20_N010k_no_overflow"
    print(f"Making plots for {working_directory}")

    dirs = getPaths(working_directory, "nodir", sim_dir="completed_simulations", existing_dir=True)
    ...
    snapshots = sorted(os.listdir(dirs["snapshots"]))
    snapshots_core = [f for f in snapshots if f.startswith("giant_core")]
    snapshots_gas = [f for f in snapshots if f.startswith("giant_gas")]
    # snapshots_core = [f for f in snapshots if f.startswith("relaxation_core")]
    # snapshots_gas = [f for f in snapshots if f.startswith("relaxation_gas")]
    snapshots_sim_gas = [f for f in snapshots if f.startswith("simulation_gas")]
    snapshots_sim_dm = [f for f in snapshots if f.startswith("simulation_dm")]
    ...
    if len(snapshots_gas) > 0:
        ...
        # filepath = os.path.join(dirs["plots"], "animation_relaxation_2D.gif")
        filepath = "testing.gif"
        animate_relaxation_2D(snapshots_core, snapshots_gas, dirs, parameters, filepath=filepath)
        # animate_density_profile(snapshots_gas, dirs, parameters, filepath=os.path.join(dirs["plots"], "animation_density_profile.gif"))
    ...
    if len(snapshots_sim_gas) > 0:
        ...
        # rows = 3
        # cols = 2
        # indeces = [0,10,20,30,40,50]
        # plot_multi_sim_density(snapshots_sim_gas, snapshots_sim_dm, rows, cols, indeces, dirs, parameters, filepath="test_multi_density.png")

        # plot_all_orbital_evolutions(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters)
        # animate_simulation_particles(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, filepath=os.path.join(dirs["plots"], "animation_simulation_particles.gif"))
        # animate_simulation_particles(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, fps=5, filepath=os.path.join(dirs["plots"], "animation_simulation_particles_slow.gif"))
        # animate_simulation(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, resolution=100, filepath=os.path.join(dirs["plots"], "animation_simulation.gif"))
        # animate_simulation_density(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, filepath=os.path.join(dirs["plots"], "animation_simulation_density.gif"))
        # animate_simulation_particles(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, filepath=os.path.join(dirs["plots"], "animation_simulation_particles_trail.gif"), do_trail=True)
        # animate_simulation_faraway(snapshots_sim_gas, snapshots_sim_dm, dirs, parameters, filepath=os.path.join(dirs["plots"], "animation_simulation_particles_far.gif"))
