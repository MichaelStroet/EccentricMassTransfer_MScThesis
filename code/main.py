...
"""
main.py is a bit of a mess.
Checks for existing files, but does not work in all cases
Also currently requires directories/files to be in a specific spot in home
/home/user/simulations
/home/user/code
/home/user/relaxed_stars
/home/user/logs
/home/user/logs/finished
All these paths are only hardcoded in main.py (search for 'HARDCODED')
Codes A, B and C are all using the relative paths of 'dirs' from directories.py.
"""
...
if __name__ == "__main__":
    ...
    # Directory containing all relaxed models    HARDCODED
    relaxed_dir = f"/home/{user}/relaxed_stars"
    # if not os.path.exists(relaxed_dir):
    #     os.mkdir(relaxed_dir)

    # Give the name of the work dir if restarting a run
    # existing_work_dir = "Mp1.5_Ms1.4_D20_e0.0_N300k"
    existing_work_dir = "work_directory_to_restart_and_grab_parameters_from"

    # Check if the above work dir exists or not   HARDCODED
    existing_work_path = os.path.join("/home", user, "simulations", existing_work_dir)
    if not os.path.exists(existing_work_path):
        print(f"\nDid not find {existing_work_path}")

        # Define new parameters
        parameters = {
            "m_primary_MSun": 1.5,              # Mass of the primary star in MSun
            "m_secondary_MSun": 1.4,            # Mass of the secondary star in MSun
            "eccentricity": 0.0,                # Eccentricity of the binary orbit
            "periapsis_RSun": 20,               # Distance at closest approach in RSun
            "roche_multiplier": 1.1,            # Multiplier of the roche radius to evolve to
            "core_mass_fraction": 0.4,        # Fraction of the primary mass for the core particle
            "n_particles": 100000,              # Total number of SPH particles
            "n_workers": 16,                    # Number of workers (processes) to run hydro
            "min_relax_time_day": 20,           # Minimum time to relax in days
            "max_relax_time_day": 5,          # Maximum time to relax in days
            "relax_timestep_day": 0.01,          # Timestep of relaxation snapshots in days
            "relaxed_ratio": 0.010,             # Ratio of initial and current 'oscillation size' to call the star relaxed
            ...
            "sink_multiplier": 3,               # Radius multiplier of the sink particle (*R_secondary)
        ...
        working_directory += "_post_update_astera_testenv"

        # dumb fix version II for not skipping evolution   HARDCODED
        home_work_dir = os.path.join("/home", user, "simulations", working_directory)
        if os.path.exists(home_work_dir):
            print(f"Found {home_work_dir}\nCopying work dir to node...\n")
            os.system(f"rsync -a {home_work_dir} $OUTPUT_FOLDER/simulations")
        ...
        ### IMPLEMENTATION IS MESSY, IDEA IS TO NOT KEEP RE-RELAXING or RE-EVOLVING THE SAME STAR
        ...
        if os.path.exists(relaxed_gas) and os.path.exists(relaxed_core):
            ...
            print(f"Star {relaxed_string} has not been relaxed before\n")

            print(dirs)

    # Otherwise, restart the run in the working directory
    else:  
        print(f"\nFound {existing_work_path}\nLoading parameters from json file")

        # load parameters from file
        parameters = load_parameter_json(os.path.join(existing_work_path, "data", "parameters.json"))
        print(f"{json.dumps(parameters, indent=4)}\n")

        # Copy the working directory to the node
        print(f"\nCopying {existing_work_dir} to node...")
        os.system(f"rsync -a {existing_work_path} $OUTPUT_FOLDER/simulations")
        print("Done\n")
    ...
    if not has_relaxed:
        ...
        ### BUGGY?
            ...
            restart_index = 0 # -1 restarts from last snapshot
